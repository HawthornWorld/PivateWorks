			...
			<div class="page-list" ref="container">
				<ul class="list">
					<li v-for="(item,idx) in pageList" :key="idx">
						<img
							:src="`//img1.raymangaapp.com${item.image_url}`"
							v-lazy="`//img1.raymangaapp.com${item.image_url}`"
							alt
						>
					</li>
				</ul>
...
export default {
	...
	created() {
		...
		let langKey = (navigator.language || navigator.userLanguage).slice(
			0,
			2
		);
		//切换ui至相应语言
...
.toggle-chapter {
	...
	.prev {
		...
		width: rem(64px);
		height: rem(64px);
		background: url(#{$base}/ic_reader_navigation_previous.png) center
